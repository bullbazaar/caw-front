...
export default function MyAvatar({ sx }: Props) {
  ...
  if (!connected) {
    return <Blockies seed={(address || '0x0').toLowerCase()} scale={5} size={8} className="rounded-full" />;
  }

  return <StoryStyledAvatar src={cawAccount?.avatar || ''} alt={cawAccount?.userName || ''} />;
}
