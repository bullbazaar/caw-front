            ...
            <Wrap>
              {links.map((link) => (
                <WrapItem key={link.name}>
                  <Link isExternal href={link.url} rel="noopener noreferrer nofollow">
                    <VStack width="sm" h="80px" bg={link.bg} p={2}>
                      {link.icon.startsWith('/icons') ? <Image src={link.icon} alt={link.name} w="24px" /> : <Iconify icon={link.icon} />}
                      <Text color={'gray.800'}>{link.translate ? t(`${link.name}`) : link.name}</Text>
                    </VStack>
                  </Link>
                </WrapItem>
              ))}
