...
function MintNFTNameForm() {
  ...
  const {
    initialized: CAWNamesMinterContractInitialized,
    getIdByUserName,
    getCostOfName,
    mint: mintCAWUsername,
  ...
  const {
    initialized: mCAWContractInitialized,
    mint: mintMCAW,
    approve: aproveMCAW,
    ...
    onTxConfirmed: ({ method }) => {
      setTxSent(false);
      if (method === 'mint') aproveMCAW(address, costCAW);

      if (method === 'approve' && !mintingCawName && debouncedValue) mintCAWUsername(debouncedValue);
    },
    onError: (err) => {
      setProcessing(false);
      const { message, code } = getBlockChainErrMsg(err);
  ...
  useEffect(() => {
    ...
    if (isMounted && !valid) setUserNameError(valid ? null : `${value} : ${t('minting_page.username_already_taken')}`);
    ...
    if (valid && !txMintHash) checkExisting();
  ...
  const handleMint = async () => {
    try {
      if (!address || !debouncedValue || !connected) throw new Error('No address');

      if (!mCAWContractInitialized || !CAWNamesMinterContractInitialized) throw new Error('Contracts not initialized, please try again later');
  ...
  return (
    <Box>
      <Stack spacing={2}>
        <Input placeholder={t('labels.entery_user')} value={value} readOnly={Boolean(txMintHash)} onChange={handleChange} />
        <WrapperFadeAnimation show={!Boolean(txMintHash) && Boolean(value)} exitDuration={0.5}>
          <AlertMessage type={isValid && !existing ? 'success' : 'warning'} variant="subtle" showIcon={isValid} message={userNameError || ''} />
        ...
        <HStack>
          ...
          {Boolean(txMintHash) && (
            <Button mt={5} width={'full'} colorScheme="blue" onClick={handleReset}>
...
export function QuickMintingUserName({ isOpen, onClose }: QuickMintingUserNameProps) {
  ...
  return (
    <>
      <Modal isCentered motionPreset="slideInBottom" isOpen={isOpen} closeOnEsc={true} closeOnOverlayClick={false} onClose={onClose}>
        <ModalOverlay />
        <ModalContent>
          ...
          <ModalFooter alignItems="center">
            <Link as={NextLink} href={PATH_AUTH.mint} color={'blue.400'} textDecoration="none">
