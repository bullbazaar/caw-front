...
import { TableContainer, Table, Thead, Tr, Th, Tbody, Td, Tooltip, TableCaption } from '@chakra-ui/react';
...
export default function ProtocolCost() {
  ...
  useEffect(() => {
    ...
    const cb = async () => {
      ...
      if (active) setCurrentMc(currentMc);
    };
  ...
  useEffect(() => {
    ...
    const cb = async () => {
      ...
      let rows = keys.map((key) => {
        ...
        const _row: any = {};
        _row.id = uuidv4();
        _row.action = t('calc.mint_action')
          .replace('{0}', length >= 8 ? '+' : '')
          .replace('{1}', key)
          .replace('{2}', length > 1 ? 's' : '');
