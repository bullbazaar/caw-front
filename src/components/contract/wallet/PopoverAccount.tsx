...
export default function PopoverAccount({ displaMode, showFooter }: PopoverAccountProps) {
  ...
  return (
    <Popover initialFocusRef={initialFocusRef as any} closeOnBlur={true}>
      <PopoverTrigger>
      ...
      <PopoverContent borderColor={popoverBordercolor} shadow="md">
        <PopoverWrapperInModal isCentered isOpen={isOpen} onClose={onClose} wrapAbove="md">
          <PopoverArrow color={popoverBordercolor} borderColor={popoverBordercolor} />
          <Show above="md">
            <PopoverCloseButton />
          </Show>
          <PopoverHeader pt={4}>
          ...
          <PopoverBody id="popover-acc-body" m={0} p={0}>
            <Flex m={2}>
              ...
              <Spacer />
              <Link as={NextLink} href={PATH_DASHBOARD.app.accounts} rel="noopener noreferrer" size="xs" colorScheme="white" variant="ghost">
                {t('labels.viewall')}
              </Link>
            </Flex>
            <chakra.div id="container" display="flex" alignItems="center" alignContent="center" width="full" height="full" overflow="hidden" p={2}>
              <Button
              ...
              >
                {cawAccounts.map((acc) => (
                  <MotionContainer key={acc.id}>
                    <Tooltip hasArrow gutter={8} placement="top" label={acc.userName} borderRadius="sm">
                      <Avatar name={acc.userName} src={acc.avatar} cursor="pointer" onClick={handleCawAccountChange(acc)}>
