...
const ConnectWalletButton = () => {
  ...
  if (status === 'disconnected') {
    return (
      <HStack id="connect-wallet-hstack" cursor={'pointer'} alignItems="center" onClick={handleOpenAccountModal}>
        <Icon viewBox="0 0 200 200" color="red.500">
          <path fill="currentColor" d="M 100, 100 m -75, 0 a 75,75 0 1,0 150,0 a 75,75 0 1,0 -150,0" />
        </Icon>
        <VStack alignItems={'flex-start'} textAlign={'left'}>
          <Text as="b">{t('wallet.notConnected')}</Text>
          <Text>{t('wallet.signIn')}</Text>
        </VStack>
  ...
  if (status === 'connecting' || status === 'reconnecting') {
    return (
      <>
        <Button variant="ghost" onClick={handleOpenAccountModal}>
  ...
  return (
    <Box>
      <VStack>
        <Button onClick={chain?.name ? openChainModal : handleOpenAccountModal} variant="ghost" bgGradient="linear(to-l, brand.100, brand.200)">
          {chain?.hasIcon && (
            <Box
              style={{
            ...
            >
              {chain?.iconUrl && <Image alt={chain.name ?? 'Chain icon'} src={chain.iconUrl} style={{ width: 12, height: 12 }} />}
            </Box>
          )}
