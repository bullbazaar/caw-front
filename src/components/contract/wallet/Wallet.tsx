...
export function WalletButton(props: WalletButtonProps) {
  ...
  if (status === 'disconnected') {
    return (
      <>
        <Button {...buttonProps} onClick={handleOpenAccountModal}>
  ...
  if (!chain || chain?.unsupported) {
    return (
      <Button {...buttonProps} onClick={openChainModal}>
  ...
  return (
    <Menu>
      ...
      <MenuList id="wallet-button-menu-list" {...menuListProps}>
        <MenuItem value="account" onClick={openAccountModal} icon={<Iconify icon="ion:wallet" width={16} height={16} />}>
          {t('labels.wallet')}
        </MenuItem>
        <MenuItem value="network" onClick={chain?.name ? openChainModal : handleOpenAccountModal} icon={<NetworkIcon />}>
          {t('labels.network')} ({chain?.name || t('labels.unknownChain')})
        </MenuItem>
        <MenuItem value="disconnect" onClick={() => disconnect()} icon={<Iconify icon="uil:exit" width={16} height={16} />}>
          {t('labels.disconnect')}
        </MenuItem>
      </MenuList>
    </Menu>
