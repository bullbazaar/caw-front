...
export default function SwapMCAWForm() {
  ...
  return (
    <Container p={5} bg={bg} w="full" minH={'container.sm'} border={'1px solid'} borderColor={brColor} borderRadius={'lg'}>
      ...
      <VStack spacing={2}>
        <FormControl isInvalid={Boolean(error)} colorScheme="blue">
          <HStack spacing={4}>
            ...
            <Spacer />
            <Text as="b">
              {t('labels.balance')} : {fetchingETH ? '...' : balance > 1 ? kFormatter(balance) : balance.toFixed(4)}
            </Text>
          </HStack>
          <Spacer h={10} />
          <VStack spacing={4}>
          ...
          <VStack alignItems="flex-end">
            ...
            <Text>{`1 CAW : $${cawUSD.toFixed(12)}`}</Text>
            <Text>{`1 ETH : $${fDecimal(ethUSD)}`}</Text>
          </VStack>

          <FormErrorMessage>{error && <AlertMessage type="warning" message={error} />}</FormErrorMessage>
          {connected ? (
            <Center>
              <VStack width={'full'}>
                <Button
            ...
            <VStack pt={10} justifyContent="center" alignItems="center" textAlign={'center'}>
              <Divider />
