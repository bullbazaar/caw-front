...
export default function AlertMessage({ type, title, message, variant, showIcon = true, maxWidth, showCloseButton = false }: Props) {
  ...
  return (
    <WrapperFadeAnimation show={isVisible} exitDuration={0.5}>
      <Alert maxWidth={maxWidth} status={type} variant={variant}>
        {showIcon && <AlertIcon />}
        <Box>
        ...
        <Spacer />
        {showCloseButton && <CloseButton alignSelf="flex-start" position="relative" right={-1} top={-1} onClick={onClose} />}
      </Alert>
