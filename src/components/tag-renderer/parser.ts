...
export const parse = ({ value, htRenderer, linkRenderer, atRenderer, stockRenderer, action }: ParserParams) => {
  ...
  const hts = value.split(rule).map((chunk: string) => {
    if (chunk.match(rule)) {
      if (chunk.includes('@')) return atRenderer(chunk, action);

      if (chunk.includes('#')) return htRenderer(chunk, action);

      if (chunk.includes('$')) return stockRenderer(chunk, action);

      if (chunk.includes('http') || chunk.includes('www')) return linkRenderer(chunk, action);
