...
function Footer() {
  ...
  return (
    <HStack width={'100%'} display="flex" alignItems="center" justifyContent="space-between" pb={4}>
      <Text as="b">{t('menu.firsttweb3')}</Text>
      <Link
        isExternal
        variant="outline"
...
export default function TopBarMenu({ textColor, iconColor, itemIconColor }: Props) {
  ...
  return (
    <Popover initialFocusRef={initialFocusRef as any} placement="bottom">
      <PopoverTrigger>
        <IconButton
          variant={'outline'}
      ...
      <PopoverContent color={textColor} borderColor={popoverBordercolor} shadow="md">
        <PopoverArrow />
        <PopoverHeader>
          <Text as="b">{t('menu.menu')}</Text>
        </PopoverHeader>
        <PopoverCloseButton />
        <PopoverBody>
          <PopoverWrapper isOpen={isOpen} onClose={onClose} wrapAbove="md" footer={<Footer />}>
            <List spacing={1}>
              <Hide above="md">
                <br />
                <WalletButton
