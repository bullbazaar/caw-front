...
function NftUsernameCard({ userId, avatar, username, onConnect }: Props) {
  ...
  const {
    contracts: {
      CAW_NAME: { address: cawNamesAddress },
    },
  ...
  return (
    ...
    >
      <Flex id={`card-${username}`} p={{ base: 4, md: 2 }} w="full" alignItems="center" justifyContent="center">
        <Box
        ...
        >
          <AnimatePresence>
            {(isVisible || isMd) && (
              ...
              >
                <chakra.a href={nftURL} target="_blank" rel="noopener noreferrer" position="absolute" top={2} right={2}>
                  {/* <Iconify icon={'simple-icons:opensea'} color="#407fdb" /> */}
                  <IconButton aria-label="OpenSea" size="sm" colorScheme="gray" icon={<Iconify icon={'simple-icons:opensea'} color="#407fdb" />} />
                </chakra.a>
                <IconButton
                  aria-label="Download"
                  position="absolute"
                  ...
                  icon={<DownloadIcon />}
                />
                <Tooltip label={`${t('labels.connectas')}: ${username}`}>
                  <IconButton
          ...
          <Image id={`image-${username}`} src={avatar} alt={`Picture of ${username}`} roundedTop="lg" />
