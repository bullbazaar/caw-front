...
function Card({ data }: { data: DataModel }) {
  return (
    <m.div initial={{ opacity: 0.3 }} whileHover={{ opacity: 0.6 }}>
      <Tag
        minWidth={CARD_WIDTH}
        maxWidth={CARD_WIDTH}
...
export default function CardParallaxSection() {
  ...
  return (
    <>
      <AnimatePresence>
        {isMd && (
          <m.div initial={{ opacity: 0, y: -100 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }}>
            <chakra.div id="title-and-text-about-caw-manifesto" position={'absolute'} w={'full'} p={2} textAlign={'center'}>
      ...
      <Box id="social-issues-section" p={0} m={0}>
        <AnimatePresence>
          {isMd && (
            <m.div initial={{ rotate: 0, scale: 1.2 }} animate={{ rotate: 0, scale: 1 }} transition={{ duration: 4 }}>
