...
function CardWrapper({ children }: { children: ReactNode; index: number }) {
  ...
  return (
    <Box mb={'8'} shadow="none" alignSelf={{ base: 'center', lg: 'flex-start' }} borderColor={borderColor} borderRadius={'xl'}>
      {children}
    </Box>
  );
}
        ...
        {CARDS.map((card, index) => (
          <m.div variants={varFade().inUp} key={card.title}>
            <CardWrapper index={index}>
              <PoweredCardInfo index={index} key={card.title} title={t(card.title)} description={t(card.description)} icon={card.icon} />
            </CardWrapper>
          </m.div>
        ))}
      </Stack>
