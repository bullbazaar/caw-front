...
export default function useMintableCAWContract(props: Props) {
  ...
  const {
    ...
    keys: { INFURA_API_KEY },
    network,
    contracts: { MINTABLE_CAW, CAW_NAME_MINTER },
  ...
  const signedContract = () => {
    if (!contract) throw new Error(CONTRACT_ERR_NOT_INIT);

    if (!allowMainnet) throw new Error(t('errors.mainnet_not_allowed'));

    if (!signer || isSignerError || loadingSigner) throw new Error(t('errors.signer_not_found'));
