...
const DAppProvider = ({ children }: Props) => {
  ...
  const handleCawAccount = useCallback(
    async (cawAccount: CawUserName, throwErr: boolean) => {
      if (!cawAccount?.userName || !address) return;

      //* Validate if the user is the owner of the username
      const _userName = userNames.find((user) => user.userName === cawAccount.userName);
      if (!_userName) {
        if (throwErr) throw new Error('You are not the owner of this username');
  ...
  useEffect(() => {
    ...
    if (!Boolean(userName) && userNames?.length > 0) handleCawAccount(userNames[0], false);
  ...
  useEffect(() => {
    if (!initialized || !address || status !== 'connected') return;

    if (!getTokenFetched) handleFetchUserNames();
      ...
      {({ account, chain, openAccountModal, openChainModal, openConnectModal, mounted }) => (
        <DAppContext.Provider
          value={{
            ...
            chain,
            connected:
              isConnected && (mounted || '').toString() !== 'loading' && Boolean(account?.address) && Boolean(chain?.id) && !chain?.unsupported,
            mounted,
