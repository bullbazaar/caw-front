...
export default function UserAcceptance() {
  ...
  return (
    <chakra.div pt={5}>
      <Text pb={2}>{t('minting_page.user_acceptance_title')}</Text>
      <Checkbox borderColor={bgCheckBox} isChecked={allChecked} isIndeterminate={isIndeterminate} onChange={handleCheckAll}>
        {t('minting_page.user_acceptance_mchk_1')}{' '}
        <Link color={'blue.400'}>
          <b>{`${t('minting_page.user_acceptance_mchk_dapp')} `}</b>
        </Link>
        {t('minting_page.user_acceptance_mchk_2')}
      </Checkbox>
      <Stack pl={6} mt={1} spacing={1}>
        {warnings.map((caption, index) => (
          <Checkbox key={caption} borderColor={bgCheckBox} isChecked={checkedItems[index]} onChange={handleCheck(index)}>
