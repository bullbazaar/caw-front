        ...
        >
          <Stack spacing={5} mx={'auto'} maxW={'full'} py={6} px={6}>
            <Stack align={'center'}>
              ...
              <NftPriceLegend />
              <AlertMessage type="info" variant="solid" title={t('labels.under_dev')} message={t('labels.testnet_msg')} showCloseButton={true} />
            </Stack>
            <Progress value={progress} colorScheme="caw" mb="5%" mx="5%" borderRadius={10} />
            <Box bg={boxBg} rounded={'lg'} boxShadow={'2xl'} p={8}>
              <Steps userName={userName} step={step} />

              <WrapperFadeAnimation show={Boolean(error)}>
                <AlertMessage type="warning" message={error} showCloseButton={true} showIcon={true} maxWidth="container.xl" />
              </WrapperFadeAnimation>

              <ButtonGroup mt="5%" w="100%">
                <Flex w="100%" justifyContent="space-between">
                  <Button isDisabled={step === 1} colorScheme="caw" variant="solid" w="8rem" mr="5%" onClick={handleBack}>
                    {t('buttons.btn_back')}
                  </Button>
                  {step !== maxSteps && (
                    <Button w="8rem" onClick={handleNext} colorScheme="caw" variant="outline" disabled={processing}>
                      {t('buttons.btn_next')}
                    </Button>
                  )}
                  {step === maxSteps && (
            ...
            <Spacer />
            <Flex direction={{ base: 'column', md: 'row' }} alignItems="center" justifyContent="center" rowGap={2}>
              <Link as={NextLink} href={PATH_DASHBOARD.app.home} color={'blue.400'} rel="noopener noreferrer">
