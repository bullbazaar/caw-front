  ...
  return (
    <Stack spacing={4} width={width <= 0 ? 'full' : width}>
      <FormControl id="username">
        ...
        <InputGroup>
          <Input ref={inputRef} size="lg" variant="filled" type="text" onChange={handleSetUsername} style={{ textTransform: 'lowercase' }} />
          <InputRightElement
            alignItems={'center'}
            children={userName ? isValid ? <CheckIcon color="green.500" /> : <CloseIcon color="red.500" /> : null}
          />
        </InputGroup>
      </FormControl>
      <WrapperFadeAnimation show={Boolean(message)} exitDuration={0.5}>
        <AlertMessage type={isValid ? 'success' : 'warning'} variant="subtle" showIcon={isValid} message={message} />
