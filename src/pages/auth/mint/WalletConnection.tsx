...
export default function WalletConnection({ onInitilizedBox }: Props) {
  ...
  useEffect(() => {
    if (refDiv?.current) {
      if (breakpoint === 'base' || breakpoint === 'sm') onInitilizedBox(0);
      else onInitilizedBox(refDiv.current.clientWidth);
    }
  }, [onInitilizedBox, breakpoint]);

  return (
    <Stack spacing={8}>
      <Box id="box-wallet-connection" textAlign="center" py={10} px={6} ref={refDiv} maxWidth="container.md">
        ...
        <Text color={fontColor1}>{t('minting_page.step_description')}</Text>
        <Heading as="h3" size="md" mt={6} mb={2}>
          {t('minting_page.nft_state_q')}
        </Heading>
        <Text color={fontColor2}>{t('minting_page.nft_state_a')}</Text>
        <Spacer h={5} />
        <Text color={fontColor3}>{t('minting_page.nft_state_ai')}</Text>
        <Spacer h={10} />
