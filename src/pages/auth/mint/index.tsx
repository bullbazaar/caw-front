...
export default function RegisterPage() {
  ...
  const {
    ...
    onTxConfirmed: ({ method }) => {
      ...
      if (method === 'approve' && !mintingCawName) mintCAWUsername(userName);
    },
    onError: (err) => {
      setProcessing(false);
      const { message, code } = getBlockChainErrMsg(err);
  ...
  const { termsAccepted, userName, message, onChainValidated, costVerified, costCAW, costETH, costUSD, swapAmount, isValid, hideBalance } =
    methods.watch();
      ...
      <Container w="full" maxW={'container.xl'} p={0}>
        ...
        >
          <FormProvider {...methods}>
            <FormStepper processing={processing} termsAccepted={termsAccepted} userName={userName} error={error} isValid={isValid} />
            <BlockChainOperationInProgressModal processing={processing} txSent={txSent} message={breakPoint} onClose={handleCloseModal} />
          </FormProvider>
        </MintingPageContext.Provider>
      </Container>
    </PageWrapper>
