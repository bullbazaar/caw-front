...
export default function UserProfilePage() {
  ...
  return (
    <PageWrapper title={t('private_profile_page.accounts')}>
      <Block title={t('private_profile_page.accounts')} subtitle={`${t('private_profile_page.accounts_subtitle')} : (${cawAccounts.length}) `}>
        <Wrap id="profiles-wrapper" justify={'space-between'}>
          {cawAccounts.map((acc) => (
            <WrapItem key={acc.userName} id={acc.userName}>
              <NftUsernameCard avatar={acc.avatar} userId={acc.id} username={acc.userName} onConnect={handleCawAccountChange} />
            </WrapItem>
          ))}
        </Wrap>
      </Block>
