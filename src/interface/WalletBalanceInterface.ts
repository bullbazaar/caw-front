...
export class WalletBalanceInterface {
  ...
  constructor(account: string, chainId: number, chainName: string, providerArg: Provider | null) {
    ...
    if (chainId === 1) this.provider = providerArg ? providerArg : new ethers.providers.JsonRpcProvider(_s.jsonRpcUrl);
    else this.provider = providerArg ? providerArg : ethers.getDefaultProvider({ chainId: this.chainId, name: this.chainName.toLowerCase() });
